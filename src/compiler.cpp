  ...
  for (size_t pc = 0; pc < (*m_tokens).size(); ++pc) {
    ...
    switch (token) {
      ...
      case Tokens::CloseBracket: {
        ...
        // bracket. We'll need to fix up the offset for that JZ, as well as emit
        // a JNZ with a correct offset back. Note that both [ and ] jump to the
        // instruction *after* the matching bracket if their condition is
        // fulfilled.

        // Compute the offset for this jump. The jump start is computed from
        // after the jump instruction, and the target is the instruction after
        // the one saved on the stack.
